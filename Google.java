...
public class Google {
	...
	public static String translate(String value, String src, String target) {
		...
		try {
			return WebRequest.get("https://translate.googleapis.com/translate_a/single?client=gtx&sl=" + src + "&tl="
					+ target + "&dt=t&q=" + WebRequest.encode(value, "UTF-8"), "utf-8");
		} catch (Exception e) {

			// e.toString();

			System.out.println("获取" + getKey((HashMap) LANGUAGE, target) + "失败，请用浏览器打开Google翻译网站,确认是否网络正常。");
		}
		return "获取失败";
	}

	/**
	 * 通过values拿到key值，使用与HashMap一对一的场景
	 * 
	 * @param map
	 * @param value
	 * @return
